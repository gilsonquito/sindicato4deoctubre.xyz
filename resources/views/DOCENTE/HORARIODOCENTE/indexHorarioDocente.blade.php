    ...
    <body>
            <div class="container-fluid p-2"><!--digeneral-->    
                    <nav class="navbar navbar-expand-lg navbar-light bg-light justify-content-center">
                        <a class="navbar-brand text-success" href="#">Horario de docente</a>
                    </nav>
            ...
            <script>
                $(document).ready(function(){
                    ...
                    $('#selPeriodoHorarios').on('change', function(e){//cargar periodo de horarios
                        $("#descarhorario").removeAttr('disabled');
                        var idTabla = e.target.value;
                        $.ajax({
                                url:'docentehorario/cargartablahorario/'+idTabla,
            ...
            <script >//cargar select dinamicos 
                ...
                $('#selPeridoAcad').on('change', function(e){//cargar periodo de horarios
                   
                    $("#descarhorario").attr('disabled','disabled');
                    var periodoacad = e.target.value;
                    
                    $.get('docentehorario/cargarminiperiodoshorarios/'+periodoacad,function(data) {
                            $('#selPeriodoHorarios').empty();
                            $('#selPeriodoHorarios').append('<option value="" disabled="true" selected="true">Elija un mini período para horario</option>');
                ...
                function descargarHorarrios()
                {
                    ...
                    window.open('descargarhorario/'+tperiodohorariog,'_blank');
                }
