...
<html lang="en">
    <head>
        ...
        <link href="{{asset('css/datosSecretaria.css')}}" rel="stylesheet">
        <title>Datos secretaría</title>
    </head>
    <body>
        <div class="container-fluid p-2"><!--digeneral-->    
            <nav class="navbar navbar-expand-lg navbar-light bg-light p-2">
                            <i class="fa fa-pencil-square-o  mr-3 lead  text-DARK" aria-hidden="true"></i>
                            <a class="navbar-brand font-weight-bold  text-warning" href="#">Datos secretaría</a>
            </nav>
            @foreach ($docentes as $docente)
            <form id="datosSecretariaForm" class="py-4">
                @csrf                         
    ...
    <!--cambiar foto-->
            ...
            <form id="avatarForm" action="{{url('/secretaria/changeImage')}}" method="POST" enctype="multipart/form-data" class="p-4">
                @csrf  
                <div class="row justify-content-center p-2">
                    <div class="col-md-6 px-2  justify-content-center text-left">
                ...
                <button type="submit" class="btn btn-warning" tile="Cambiar fto" id="btnCambiarFoto"><i class="fa fa-pencil mr-2" aria-hidden="true"></i> Cambiar Foto</button>
            </form>
        </div>
        <script>//actualizar cambios realizados                
                $('#datosSecretariaForm').submit(function(e){
                    e.preventDefault();
                    ...
                    $.ajax({
                                url:"{{route('secretaria.actualizarSecretaria')}}",
                                type:"POST",
                                data:{
                                        id_doc:id_doc2,                                        
                                        name:name2,                                       
